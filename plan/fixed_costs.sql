...
-- ─────────────────────────────────────────────────────────────
-- 【追記】fixed_costs に未登録だが毎月落ちている支払い
-- cashflow_transactions（銀行明細 2026/01-05）から検出。すべて毎月・定日。
--   那覇会議所会費      18,000
--   大同火災（保険）    16,470
--   ソニー生命(SMBC)     8,140
--   TGひかり(SMCC)       6,827
...
--   小計                54,937   → 固定費に加算すべき
--
-- 固定費 合計 = 5,403,000 + 54,937 = 5,457,937 円/月（年 65,495,244）
--   実費         4,917,937
--   積立・利益枠    540,000
--
-- 【別枠】借入返済（費用ではないが毎月必ず出る現金）
--   日本政策金融公庫（シヨウカン キンユウコウコ）223,734
--   返済（もう1本）                               71,656
--                                               ───────
--   小計                                         295,390 円/月（年 3,544,680）
--   → 元本部分は税引後利益から返済するため、「税引後2,000万を残す」目標のうち
--     354万が返済に消える。実際に積める守りの金は 1,646万。
--
-- 毎月出ていく現金の総額 = 5,457,937 + 295,390 = 5,753,327 円/月
